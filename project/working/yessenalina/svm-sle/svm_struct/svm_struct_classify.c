...
int main (int argc, char* argv[])
{
    ...
    */
    ...
    init_struct_model(&testsample, &model, &sparm, NULL, NULL);
    ...
    if(struct_verbosity>=1) {
        ...
        {
            printf("Inferring latent assignments..."); fflush(stdout);
        }
    }
    ...
    for(i=0;i<testsample.n;i++) {
        ...
        if(infer_latent == 0)
            ...
            y=infer_hidden_variables(testsample.examples[i].x, testsample.examples[i].y, &model, &sparm);
        runtime+=(get_runtime()-t1);

        if(infer_latent == 0)
        {
            if(classify_verbose == 1)
                write_label_verbose(predfl,y);
            else
        ...
        long *verbosity,long *struct_verbosity)
{
    ...
    for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
        ...
        { 
            ...
            case 'l': infer_latent = 1; break;
            case 'a': classify_verbose = 1; break;
    ...
    struct_parm->classify_verbose = classify_verbose;
...
void print_help(void)
{
    ...
    copyright_notice();
    printf("   usage: svm_sle_classify [options] example_file model_file output_file\n\n");
    printf("options: -h         -> this help\n");
    printf("         -v [0..3]  -> verbosity level (default 2)\n");
    printf("         -a         -> output all scores (classify verbose)\n");
    printf("                       prints scores of each sentence of each example:\n");
    printf("                       [polar] [subj] [joint pos] [joint neg]\n");
    printf("         -l         -> infer latent variables (instead of classify)\n");
    printf("                       This implements Line 7 of Algorithm 2 in [1]\n");
    printf("                       see run_training.py for example of how to use\n");
    ...
    printf("    Empirical Methods in Natural Language Processing, 2010.\n");
    printf("\n");
}
