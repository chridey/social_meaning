			   ...
			   int *alg_type)
{
  ...
  /* set default */
  struct_parm->model_loaded = 0;
  struct_parm->feature_mode = DEFAULT_MODE;
  struct_parm->latent_size_mode = DEFAULT_LATENT_SIZE;
  struct_parm->norm_mode = DEFAULT_LATENT_NORM;
  struct_parm->window_mode = 0;
  struct_parm->model_prior_file = NULL;
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      ...
      case 'p': {
                    i++;
                    struct_parm->model_prior_file = malloc(sizeof(char)*1000);
                    strcpy (struct_parm->model_prior_file, argv[i]);
                    break;
                }
      case 'm': i++; struct_parm->feature_mode=atoi(argv[i]);break;
      case 'l': i++; struct_parm->latent_size_mode=atoi(argv[i]);break;
      case 'n': i++; struct_parm->norm_mode=atoi(argv[i]);break;
      case 'i': struct_parm->window_mode = 1; break;
  ...
  strcpy (struct_parm->latent_init_file, argv[i+1]);
  if((i+2)<argc) {
...
void print_help()
{
  printf("\nSVM-sle learning module: %s, %s, %s\n",INST_NAME,INST_VERSION,INST_VERSION_DATE);
  printf("   extends SVM-struct %s for learning complex outputs, %s\n",STRUCT_VERSION,STRUCT_VERSION_DATE);
  printf("   includes SVM-light %s quadratic optimizer, %s\n",VERSION,VERSION_DATE);

  printf("\nNOTE: this program only implements 1 iteration of the training procedure as described in\n");
  printf("      Algorithm 2 of [1] ( corresponds to SSVMSolve ).\n");
  printf("      See run_training.py for example of how to train for multiple iterations.\n");

  copyright_notice();
  printf("   usage: svm_sle_learn [options] example_file latent_init model_file\n\n");
  printf("Arguments:\n");
  printf("         example_file-> file with training data\n");
  printf("         latent_init -> file containing sles (latent var initialization)\n");
  ...
  printf("         -m [0..3]   -> feature modes (default %d)\n", DEFAULT_MODE);
  printf("                        0 -- flat model\n");
  printf("                        1 -- sentence-only model as described in Eq.(3) in [1]\n");
  printf("                        2 -- combines modes 0 + 1 \n");
  printf("                        3 -- feature smoothing model as described in Section 4.5 in [1]\n");
  printf("         -l [num]    -> extraction size mode (default is %d)\n",DEFAULT_LATENT_SIZE);
  printf("                        corresponding to different modes of selecting the extraction size constraint\n");
  printf("                        as described in Eq.(5) in [1]. \n");
  printf("                        -- for 1 <= num <= 99, interpreted as a percentage of number of sentences,\n");
  printf("                           rounded to the nearest integer, and lower capped at 1.\n"); 
  printf("                        you can implement more options by modifying latent_size_custom.cc\n");
  printf("         -n [num]    -> extraction norm mode (default is %d)\n", DEFAULT_LATENT_NORM);
  printf("                        see Eq.(3) and Section 4.3 in [1] for more details.\n");
  printf("                        0 -- no normalization\n");
  printf("                        1 -- 1 / extraction size \n");
  printf("                        2 -- 1 / sqrt( extraction size )\n");
  printf("                        you can implement more options by modifying latent_size_custom.cc\n");
  printf("         -i          -> contiguous window mode (default is off)\n");
  printf("         -p [file]   -> regularize relative to a prior model in file (default is the 0 vector)\n");
  ...
  printf("\nMore details in:\n");
  printf("[1] A. Yessenalina, Y. Yue, C. Cardie, Multi-level Structured Models for\n");
  printf("    Document-level Sentiment Classification, Proceedings of the Conference on\n");
  printf("    Empirical Methods in Natural Language Processing (EMNLP), 2010.\n");
  printf("[2] C.-N. Yu, T. Joachims, Learning Structural SVMs with Latent Variables, \n");
  printf("    Proceedings of the International Conference on Machine Learning (ICML), 2009.\n");
  printf("[5] T. Joachims, T. Finley, Chun-Nam Yu, Cutting-Plane Training of Structural\n");
